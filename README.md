...
@xinfin/xrc-token-registry

[![npm](https://img.shields.io/npm/v/@xinfin/xrc-token-registry)](https://unpkg.com/@xinfin/xrc-token-registry@latest/) [![GitHub license](https://img.shields.io/badge/license-APACHE-blue.svg)](https://github.com/xinfin-labs/token-list/blob/b3fa86b3fdd9c817139e38641d46c5a892542a52/LICENSE)

xinfin Token Registry is a package that allows application to query for list of tokens.
...
```bash
npm install @xinfin/xrc-token-registry
```

```bash
yarn add @xinfin/xrc-token-registry
```
...
import { TokenListProvider, TokenInfo } from '@xinfin/xrc-token-registry';
...
To add a new token, add another json block to the large `tokens` list in `src/tokens/xinfin.tokenlist.json` and submit a PR.

Tips:
* `logoURI` 
  * should point to a `png`, `jpg`, or `svg`.
  * the logo can be hosted in this repo in `assets/mainnet/TOKEN_ADDRESS/FILE` 
    * in that case, the image should be added to this repo and logoURI should point to `https://raw.githubusercontent.com/xinfin-labs/token-list/main/assets/mainnet/TOKEN_ADDRESS/FILE`)
...
```
git remote add pub-origin git@github.com:xinfin-labs/token-list.git
git fetch pub-origin main 
git rebase pub-origin/main
git push origin main -f
```

More generally, for modifications to existing tokens, be sure to checkout the `HEAD` of the `main` branch, locate the existing block in `xinfin.tokenlist.json`, and modify the appropriate fields.

Always check the 'Files changed' tab on your PR to see the impact of your change.


## Scanner/wallet hasn't updated yet
Solscan, xinfin explorer, and wallets all pull from this repo at different cadences.  Some update every few days.  
...
are done with the xinfin Foundation's ("SF") good faith efforts. It is up to
...
applications (including the code comprising the xinfin blockchain
...
bypass prohibitions, obtain the code comprising the xinfin blockchain
protocol (or other project code or applications) and deploy, integrate,
or otherwise use it. Accordingly, there is a risk to individuals that
other persons using the xinfin blockchain protocol may be sanctioned
persons and that transactions with such persons would be a violation of
U.S. export controls and sanctions law. This risk applies to
individuals, organizations, and other ecosystem participants that
deploy, integrate, or use the xinfin blockchain protocol code directly
(e.g., as a node operator), and individuals that transact on the xinfin
